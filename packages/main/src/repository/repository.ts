...
export class Repository {
    ...
    public async createWorkspace(workspace: Workspace): Promise<Workspace> {
        ...
        return new Promise((resolve, reject) => {
            this.db.get('INSERT INTO workspaces (path, name) VALUES (?, ?) returning *', [workspace.path, workspace.name], (err, row) => {
                if (err) {
                    reject(err);
                } else {
    ...
    public async listWorkspaces(paginationOption?: PaginationOptions): Promise<Workspace[]> {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.db.all('SELECT * FROM workspaces ORDER BY last_used_at DESC LIMIT ? OFFSET ?', [limit, offset], (err, rows) => {
                if (err) {
                    reject(err);
                } else {
