import { AsyncSubscription, Event, subscribe } from "@parcel/watcher";
import { readFile } from "fs/promises";
import { glob } from "glob";
import { promisify } from "util";
import { Directory, DirectoryTreeEvent } from "../../../common/directory-tree";
...
export class DirectoryTree {
    ...
    public async explore(dirToExplore?: string): Promise<Directory> {
        ...
        let all = await globAsync(pattern, { cwd: this.dirPath })
        let files = await globAsync(pattern, { cwd: this.dirPath, nodir: true })
        let children: Directory[] = [];
        for (const file of files) {
            let tmpId = all.indexOf(file)
            all.splice(tmpId, 1)
        }
        all.forEach((file) => {
            children.push({
