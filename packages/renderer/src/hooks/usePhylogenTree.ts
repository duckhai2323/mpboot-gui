...
const initialUsePhylogenTree = (): [newick: string, setNewick: (str: string) => void, subscribeCommand: (id: string) => void] => {
    return ['', (_str: string) => {return;}, (_id: string) => {return;}];
};

const usePhylogenTreeImpl = (): [newick: string, setNewick: (str: string) => void, subscribeCommand: (id: string) => void] => {
