...
export const MatrixTable = memo((props: MatrixTableProps) => {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      if (!cell) return;
      if (lastHighlightCellRef.current) lastHighlightCellRef.current.classList.remove('highlighted');
    ...
    <>
      <div className="phylip-content-view unselectable">
        ...
        >
          ...
          >
            <tbody>
              <MajorRows majors={matrixToRender.majors} currentRight={matrixToRender.currentRight}/>
              <MatrixRows matrix={matrixToRender.matrix} majors={matrixToRender.majors} currentRight={matrixToRender.currentRight} />
            </tbody>
          </table>
        </Scrollbars>
      </div>
...
const MajorRows = memo(({ majors }: { majors: PhylipMatrix['majors'], currentRight: number }) => {
  return (
    <tr>
  ...
  console.log("majors", prevProps.currentRight, nextProps.currentRight)
  return prevProps.currentRight === nextProps.currentRight;
});

const MatrixRows = memo(({ matrix, majors, currentRight }: { matrix: PhylipMatrix['matrix'], majors: PhylipMatrix['majors'], currentRight: number }) => {
  return (
    <>
      {matrix.map((line, row) => (
...
export const NameTables = memo((props: NameTablesProps) => {
  const { names } = props;
  return (
    <div>
