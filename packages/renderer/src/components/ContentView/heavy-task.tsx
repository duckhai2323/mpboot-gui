...
export const createHeavyElement = (content: string) => {
  ...
  const createElement = (
    ...
    if (options?.classNames) {
      elem = `<${tag} ${!!options?.id && `id="${options.id}"`} class="${options.classNames.join(
        ' ',
      )}">${inner}</${tag}>`;
    } else {
  ...
  const namesTable = createElement(
    'table',
    createElement(
      'tbody',
      createElement(
        ...
        createElement('td', 'Names') +
          matrix.names.map(name => createElement('tr', createElement('td', name))).join(''),
      ),
    ),
    {
  ...
  const matrixTables = createElement(
    'table',
    createElement(
      'tbody',
      createElement(
          ...
          .map(line =>
            createElement(
              ...
              line
                .split('')
                .map((char, i) =>
                  createElement('td', char, {
                    classNames: [
                      'cell',
                      'cell-on-screen',
                      char === '.' ? `dot ${matrix.majors[i]}` : char,
                    ],
                  }),
                )
                .join(''),
...
export const createReactElementFromPhylipMatrix = (matrix: PhylipMatrix) => {
  return (
    <div className="phylip-content-view">
      ...
      <div
        className="content-table unselectable"
        onScroll={onScroll}
      >
        <table>
          <tbody>
              ...
              <tr>
                {line.split('').map((char, index) => (
                  <td
                    className={
                      char === '.'
                        ? `${matrix.majors[index]} dot cell cell-off-screen`
                        : `${char} cell cell-off-screen`
                    }
                  >
                    {char}
                  </td>
